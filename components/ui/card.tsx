...
const Card = React.memo(function Card({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      data-slot="card"
      className={cn(
        'bg-background text-foreground flex flex-col rounded-xl border shadow-sm py-0 gap-0 border-none',
        className,
      )}
      {...props}
    />
...
const CardContent = React.memo(function CardContent({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('px-4 py-4 border-none bg-background', className)}
      {...props}
    />
  )
})
